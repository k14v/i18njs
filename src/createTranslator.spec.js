...
const locales = {
  es: {
    ...
    'El %d%% de los moviles son moviles inteligentes':
      'El %d%% de los moviles son moviles inteligentes',
    'Tengo %d gato': {
      one: 'Tengo un gato',
  ...
  en: {
    ...
    Teléfono: 'Phone',
    Perro: 'Dog',
    'Por un beso de tu boca %d caricias te daría':
      'For a kiss from your mouth %d caresses would give you',
  ...
  de: {
    ...
    Perro: 'Hund',
    'Por un beso de tu boca %d caricias te daría':
      'Für einen Kuss aus dem Mund würden dir mehrere Liebkosungen geben',
    'El %s le dije a mi mujer que nos casasemos':
      'Am %s habe ich meiner Frau erzählt, dass wir geheiratet haben',
    'El %d%% de los moviles son moviles inteligentes': '%d%% der Handys sind Smartphones',
    'Tengo %d gato': {
...
test('it should increase at least 10 times the perfomance when try to translate the same literal in large catalog', (t) => {
  const catalog = [
    ...new Array(5000)
      .fill()
      .map(() => 'foo %s bar %d ' + (~~(Math.random() * 0xffffff)).toString(16)),
